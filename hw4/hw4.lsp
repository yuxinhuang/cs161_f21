...
(defun at-least-one-color (n c k)
	(cond
		((> c k) '())
		;a list of possible colors for node n
		(t (cons (node2var n c k) (at-least-one-color n (+ c 1) k)))
	)
  )

;helper function for at-most-one-color
;for each clauses of CNF
;for each pair of variables, there is at least one false variable
(defun at-most-one-color-helper (n c k base)
	(cond
		((>= c k) '())
		(t (cons (list (* -1 (node2var n base k)) (* -1 (node2var n (+ c 1) k))) (at-most-one-color-helper n (+ c 1) k base)))
	)
...
(defun at-most-one-color (n c k)
	(cond
		((> c k) '())
		(t (append (at-most-one-color-helper n c k c) (at-most-one-color n (+ c 1) k)))
	)
...
;match up the at-least-one-color for x and y
(defun generate-edge-clauses-helper (x y)
	(cond
		((null x) '())
		(t (cons (list (* -1 (car x)) (* -1 (car y))) (generate-edge-clauses-helper (cdr x) (cdr y))))
...
(defun generate-edge-clauses (e k)
	(let ((x (at-least-one-color (car e) 1 k)) (y (at-least-one-color (cadr e) 1 k)))
		(generate-edge-clauses-helper x y)
	)
